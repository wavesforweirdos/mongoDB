...
db.restaurants.find({'grades.score':{$lt:10}}).forEach(function(x){print(`${x.restaurant_id} - ${x.name} - ${x.borough} - ${x.cuisine}`)});
//or
db.restaurants.find({grades:{$elemMatch:{score:{$lt:10}}}}).forEach(function(x){print(`${x.restaurant_id} - ${x.name} - ${x.borough} - ${x.cuisine}`)});
...
db.restaurants.find({'address.street':{$exists:null}}) //devuelve los que NO 'street' en 'address'
db.restaurants.find({'address.street':{$ne:null}}) //devuelve los que SI tienen 'street' en 'address'
